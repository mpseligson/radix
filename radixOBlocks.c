 ...
 *
 * The radixOBlocks.c class implements R2, the third iteration on the parallelized radix sorting algorithm.
 */
...
int main(int argc,
  ...
  for (int size = 1; size < 20000000; size *= 2) {
    ...
    for (int run = 0; run < runs; run++) {
      ...
      for (int exp = 1; m / exp > 0; exp *= 10) { // m/exp > 0
        ...
        // keep track of size of blocks to be received
        int blockReceive[10] = {
          0
        };
        for (int i = 0; i < world_size; i++) {
          ...
          // store size in buf, then write buf to blockReceive for source processor
          int buf;
        ...
        // send and receive blocks of specified size
        for (int i = 0; i < world_size; i++) {
